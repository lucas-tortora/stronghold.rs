...
// if you see this module `actor_tests`, remove it completely
#[cfg(test)]
mod actor_tests {

    use actix::{Actor, Addr, Handler, Message, System, SystemRunner};
    ...
    #[derive(Default)]
    struct RegistryActor<A>
    where
        A: Actor,
    {
        clients: HashMap<usize, Addr<A>>,
    }

    #[derive(Message)]
    #[rtype(result = "Addr<Client>")]
    ...
    // default actor impl for registry actor
    impl<A> Actor for RegistryActor<A> where A: Actor {}

    impl<A> Handler<RegistryEntry<A>> for RegistryActor<A>
    where
        A: Actor,
    {
        type Result = Option<Addr<Client>>;

        fn handle(&mut self, msg: RegistryEntry<A>, ctx: &mut Self::Context) -> Self::Result {
            match msg {
                RegistryEntry::Get(id) => {
                    if let Some(result) = self.clients.get(msg.id) {
                        return result;
                    }
                    None
                }
                RegistryEntry::Put(id, addr) => {
    ...
    #[test]
    fn test_comm_of_multiple_actors_of_same_type() {
        let system = System::new();

        let registry_actor = RegistryActor::default();
        let re_actor = registry_actor.start();

        for i in 0..12 {
            let client = Client::new(i);

            system.block_on(re_actor.send(RegistryEntry::Put(i, client.start())));
        }

        let client_a = system.block_on(re_actor.send(RegistryEntry::Get(2)));

        system.block_on(client_a.send(Ping {}));
    }
}
